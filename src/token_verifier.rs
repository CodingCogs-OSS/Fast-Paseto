...
impl TokenVerifier {
    ...
    pub fn v4_public_verify(
        ...
        let verifying_key = VerifyingKey::from_bytes(&key_bytes)
            .map_err(|e| PasetoError::InvalidKeyFormat(format!("Invalid Ed25519 public key: {}", e)))?;
        ...
        let payload_bytes = URL_SAFE_NO_PAD
            .decode(parts[2])
            .map_err(|e| PasetoError::InvalidTokenFormat(format!("Invalid base64url payload: {}", e)))?;
        ...
        let token_footer = if parts.len() == 4 {
            Some(
                URL_SAFE_NO_PAD
                    .decode(parts[3])
                    .map_err(|e| PasetoError::InvalidTokenFormat(format!("Invalid base64url footer: {}", e)))?,
            )
        } else {
            None
        };
...
mod tests {
    ...
    fn test_v4_public_verify_with_footer() {
        ...
        // Generate token
        let token = TokenGenerator::v4_public_sign(&keypair.secret_key, payload, Some(footer), None).unwrap();
    ...
    fn test_v4_public_verify_with_implicit_assertion() {
        ...
        // Generate token
        let token = TokenGenerator::v4_public_sign(&keypair.secret_key, payload, None, Some(implicit)).unwrap();
    ...
    fn test_v4_public_verify_invalid_key_format() {
        ...
        assert!(result.is_err(), "Invalid or zero key should fail verification");
    }
    ...
    fn test_v4_public_verify_wrong_key() {
        ...
        let token = TokenGenerator::v4_public_sign(&keypair1.secret_key, payload, None, None).unwrap();

        // Try to verify with keypair2's public key
        let verifier = TokenVerifier::new(None);
        let result = verifier.v4_public_verify(&token, &keypair2.public_key, None, None);
        assert!(matches!(result, Err(PasetoError::SignatureVerificationFailed)));
    }
    ...
    fn test_v4_public_verify_footer_mismatch() {
        ...
        // Generate token with footer1
        let token = TokenGenerator::v4_public_sign(&keypair.secret_key, payload, Some(footer1), None).unwrap();
    ...
    fn test_v4_public_verify_implicit_assertion_mismatch() {
        ...
        // Generate token with implicit1
        let token = TokenGenerator::v4_public_sign(&keypair.secret_key, payload, None, Some(implicit1)).unwrap();
